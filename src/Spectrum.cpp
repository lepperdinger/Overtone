...
Spectrum::Spectrum(const WAVE &wave,
        ...
        frequencies(std::make_shared<Vector>())
{
    ...
    {
        throw std::invalid_argument("0 <= key_range.first < key_range.second "
                                    "<= 88 not fulfilled.");
    }
    ...
    auto unique_end = std::unique(unique_channels.begin(),
                                  unique_channels.end());
...
void Spectrum::evaluate_frame()
{
    ...
    Vector all_frequencies = evaluate_all_frequencies(time_range,
                                                      wave.get_sample_rate());
    VectorRange frequency_range = key_range_to_frequency_range(key_range,
                                                               all_frequencies);

#ifdef DEBUG
    if (frequency_range.first > std::numeric_limits<int>::max())
    {
        throw std::overflow_error("Unsigned `frequency_range.first` cannot "
                                  "be converted to signed int.");
    }
    if (frequency_range.second > std::numeric_limits<int>::max())
    {
        throw std::overflow_error("Unsigned `frequency_range.second` "
                                  "cannot be converted to signed int.");
    }
#endif
    ...
    spectrum = std::make_shared<Vector>(evaluate_spectrum(wave.get_signal(),
                                                          channels,
                                                          time_range,
                                                          frequency_range));
}
...
Spectrum::VectorRange Spectrum::evaluate_time_range()
{
    ...
    else
    {
        ...
        {
            time_range.first = time_range_video_frame.first
                               - half_missing_samples;
        }
        else
        {
            time_range.first = 0;
        }
        VectorSize new_index = time_range_video_frame.second
                               + half_missing_samples;
...
Spectrum::Vector Spectrum::evaluate_spectrum(
...
Spectrum::Vector Spectrum::evaluate_channel_spectrum(
         ...
         ++frequency_index)
    {
             ...
             ++time_index)
        {
            ...
            fourier_negative_imaginary_part += (
                current_sample * sin(constant * frequency_index_double
                                     * time_index)
            );
            fourier_real_part += (
                current_sample * cos(constant * frequency_index_double
                                     * time_index)
            );
        }
        spectrum.push_back(
...
Spectrum::Vector Spectrum::evaluate_all_frequencies(
    const VectorRange &time_range,
...
)
{
    ...
    for (VectorSize frequency_index = 0;
         frequency_index != number_of_samples / 2;
         ++frequency_index)
    {
...
Spectrum::VectorRange Spectrum::key_range_to_frequency_range(
...
)
{
    ...
    auto lower_iterator = std::find_if(
        all_frequencies.cbegin(), all_frequencies.cend(),
        [&lower_frequency] (const double &frequency)
        {
            return lower_frequency < frequency;
        }
    );
    auto upper_iterator = std::find_if(
        all_frequencies.cbegin(), all_frequencies.cend(),
...
double Spectrum::sqr(double value)
{
    return value * value;
}
