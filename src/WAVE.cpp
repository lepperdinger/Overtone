...
void WAVE::read(std::ifstream &file,
...
void WAVE::search_chunk(std::ifstream &file,
...
//    search_chunk(file, chunk_id, chunk_size);
    ...
    if (!file)
    {
        throw WAVE::parsing_error("Parsing of the chunk '"
                                  + chunk_id + "' failed.");
    }
}
...
void WAVE::parse_format_chunk(std::ifstream &file)
{
    ...
    {
        throw WAVE::parsing_error("The size of the format chunk isn't 16 "
                                  "bytes.");
    }
...
void WAVE::show_file_contents() const
{
    // RIFF Chunk
