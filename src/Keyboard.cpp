...
Keyboard::Keyboard(std::initializer_list<Spectrum> spectra):
        spectra(spectra),
        keyboard(std::make_shared<Vector>())
{
    evaluate_keys();
}
...
void Keyboard::evaluate_keys()
{
    ...
    KeyRange key_range;
    double weighted_spectrum_accumulator,
           weight_accumulator,
           weight;
    Vector::const_iterator key,
                           keys_end,
                           spectrum_value;
    for (const Spectrum &spectrum : spectra)
    {
             ...
             assigned_key != key_range.second; ++assigned_key)
        {
            ...
            {
                (*keyboard)[assigned_key] += weighted_spectrum_accumulator
                                             / weight_accumulator;
            }
        }
    }
